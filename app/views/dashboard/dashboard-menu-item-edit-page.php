...
<form method="post">
	...
	<?php foreach((array)$this->menuStorage['categories'] as $categories): ?>
    ...
    <input type="text" id="item-name" name="name" value="<?=$this->escapeForAttributes($this->menuStorage['name']);?>" required>
    <label for="item-price">Price</label>
    <input type="number" id="item-price" name="price" value="<?=$this->escapeForAttributes($this->menuStorage['price']);?>" step="0.01" min="0" max="99.99" required>
    <label for="item-description">Description</label>
    <textarea id="item-description" name="description" cols="40" rows="5" required><?=$this->escapeForAttributes($this->menuStorage['description']);?></textarea>
    <input type="hidden" id="menu-item-id" name="id" value="<?=(int)$this->menuStorage['id'];?>">
...
<div id="choices-container" class="choices-container">
    <?php foreach((array)$this->menuStorage['choices'] as $groupID => $choiceGroup): ?>
	<div id="<?=$groupID;?>-choice-group" class="choice-group">
	    ...
	    <?php foreach((array)$choiceGroup['options'] as $choiceID => $choice): ?>
		<div id="<?=$choiceID;?>-choice-option" class="choice-option">
		    ...
		    <input type="number" id="<?=$choiceID;?>-option-price" class="option-price" name="price" value="<?=$this->escapeForAttributes($choice['price_modifier']);?>" step="0.01" min="0" max="99.99" required>
		</div>
	    <?php endforeach; ?>
	</div>
    <?php endforeach; ?>
...
<select name="additions" id="addition-select-list" required>
    <?php foreach((array)$this->menuStorage['all_additions'] as $additions): ?>
	<option value="<?=$additions['id']?>">
	    <?=$this->escapeForHTML($additions['name']) . ' - ' . $additions['price_modifier']?>
	</option>
    <?php endforeach; ?>
</select>
...
<div id="additions-container" class="additions-container">
    <?php foreach((array)$this->menuStorage['additions'] as $addition): ?>
	<div class="addition" id="<?=$addition['id']?>-addition">
	    <p>
		<?=$this->escapeForHTML($addition['name']) . ' - ' . $addition['price_modifier'];?>
	    </p>
